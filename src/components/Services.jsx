  ...
  return (
    <div className="container mx-auto py-12">
      ...
      <section className="relative bg-gray-900 text-white h-64 flex items-center justify-center mb-12" style={{ backgroundImage: 'url(https://via.placeholder.com/1500x400)', backgroundSize: 'cover', backgroundPosition: 'center' }}>
        ...
        <h1 className="relative z-10 text-3xl sm:text-4xl md:text-5xl font-bold px-4 text-center">Our Drone Services</h1>
      </section>

      {/* Service Cards */}
      <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10 mb-12 px-4">
        {services.map((service, index) => (
          <div key={index} className="bg-white p-8 rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300">
            <h3 className="text-2xl sm:text-3xl font-bold mb-4 text-gray-800">{service.title}</h3>
      ...
      <section className="py-16 bg-blue-700 text-white text-center rounded-lg shadow-xl px-4">
        <h2 className="text-3xl sm:text-4xl font-bold mb-6">Ready to Elevate Your Operations?</h2>
        <p className="text-md sm:text-xl mb-8">Discover how SHLOKA's drone services can benefit your project.</p>
        <button className="bg-white text-blue-700 font-semibold px-6 py-3 sm:px-8 sm:py-4 rounded-lg text-lg hover:bg-gray-100 transition-colors duration-300">Contact Us for Services</button>
      </section>
    </div>
  );
};
